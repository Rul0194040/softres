...
export class second1629047779237 implements MigrationInterface {
  name = 'second1629047779237';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP TABLE IF EXISTS \`softres\`.\`insumos\``);
    await queryRunner.query(
      `CREATE TABLE \`softres\`.\`insumos\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` char(36) NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`active\` tinyint NOT NULL DEFAULT 1, \`nombre\` varchar(100) NOT NULL, \`unidad\` varchar(100) NOT NULL, \`marca\` varchar(100) NOT NULL, UNIQUE INDEX \`IDX_8660e0a51b002f2afa8fed1e76\` (\`uuid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`softres\`.\`almacen\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` char(36) NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`active\` tinyint NOT NULL DEFAULT 1, \`type\` enum ('PROMEDIOS', 'UEPS', 'PEPS') NOT NULL DEFAULT 'PROMEDIOS', \`unidad\` varchar(25) NOT NULL, \`insumoId\` int NULL, UNIQUE INDEX \`IDX_f5261de086b3c1e396804ff07a\` (\`uuid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`softres\`.\`almacen-Detalle\` ADD CONSTRAINT \`FK_d58da4be866de5215426c3a5052\` FOREIGN KEY (\`almacenId\`) REFERENCES \`softres\`.\`almacen\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }
